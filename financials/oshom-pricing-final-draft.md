# Oshom Pricing — Complete Logic (All IDR)

> **Date:** March 4, 2026
> **Status:** Internal draft for Tom + Selina review before presenting to Daisy
> **All figures in IDR unless stated otherwise**

---

## 1. How the Pricing Chain Works

Every product follows the same chain:

```
Step 1: Oshom net price        (what Oshom keeps after tax)
Step 2: × 1.21                 (Oshom's 21% tax & service charge)
Step 3: = BUY price            (what BaliLove pays Oshom)
Step 4: ÷ 0.85                 (add BaliLove's 15% commission)
Step 5: = SELL price            (what the couple pays)
```

**Total markup on Oshom net: × 1.4235 (42.35%)**

Example: If Oshom nets 1,000,000 → Buy = 1,210,000 → Sell = 1,423,529

---

## 2. Oshom Baseline Data (Their Actual Numbers)

From Oshom's 2025 actuals and 2026 budget:

| Metric | 2025 Actual | 2026 Budget |
|--------|------------|------------|
| Rooms | 18 | 18 |
| Occupancy | 47.4% | 61.0% |
| ADR (avg daily rate, per room) | 3,308,732 | 4,028,398 |
| Room revenue (annual) | 9,466,282,006 | 16,099,491,042 |
| Avg revenue per night (all rooms) | 25,935,019 | 44,108,879 |
| Banquet revenue (annual) | 877,015,079 | 1,842,500,000 |

**Weakest months (2025 occ.):** Nov 29.8%, Apr 34.4%, Mar 35.3%, Jan 39.5%

Revenue per night on weakest months: ~13,500,000 - 22,000,000

---

## 3. Oshom Published Rates (From Their Sales Materials)

| Product | Net Rate | Incl 21% Tax |
|---------|----------|-------------|
| Accommodation buyout | 153,000,000 /night (18 rooms) | 185,130,000 /night |
| = Per room/night | 8,500,000 | 10,285,000 |
| Venue buyout (event spaces) | 100,000,000 /day | 121,000,000 /day |
| Food — Family Style (3-course) | 600,000 /pax | 726,000 /pax |
| Food — BBQ Family Style | 850,000 /pax | 1,028,500 /pax |
| Food — Western Buffet | 750,000 /pax | 907,500 /pax |
| Food — Balinese Buffet (50+ pax) | 400,000 /pax | 484,000 /pax |
| Drinks — Bronze (beer/soft) | 400,000 /pax | 484,000 /pax |
| Drinks — Silver (+ spirits) | 550,000 /pax | 665,500 /pax |
| Drinks — Gold (+ cocktails) | 650,000 /pax | 786,500 /pax |
| Drinks — Luxe (+ wine) | 750,000 /pax | 907,500 /pax |

---

## 4. Vedas & BBG Benchmarks (BaliLove's Top Venues)

### Villa Vedas — BaliLove's #1 Venue

Villa Vedas: 7 bedrooms (sleeps 16), 73 confirmed bookings for 2026.

> **Correction (March 5):** Guests do NOT pay for accommodation at Vedas. The villa buyout includes all rooms — there is no guest offset. Previous estimates overstated F&B rates.

**Actual wishlist pricing (AUD sell, 2-night, 80 guests):**

| Component | AUD | IDR (×10,600) |
|-----------|-----|---------------|
| Villa buyout (2 nights) | $6,272 | 66,483,000 |
| Event fee | $3,814 | 40,428,000 |
| Banjar fee | $339 | 3,593,000 |
| F&B food (Western Buffet, 80 pax) | $6,442 | 68,285,000 |
| F&B drinks (Triple Spirits, 80 pax) | $5,764 | 61,098,000 |
| **Total venue + F&B** | **$22,631** | **239,887,000** |
| Guest accommodation offset | $0 | — |
| **Couple venue OOP** | **$22,631** | **239,887,000** |

- Per bedroom/night: A$3,136/night (A$448/bedroom/night)
- F&B per head: A$153 (food $81 + drinks $72) = ~1,622,000 IDR
- Min F&B spend: 40M IDR net per night
- External caterer (Saffron Bali)
- Max 16 guests on-site — everyone else stays elsewhere

### Bali Beach Glamping (BBG)

BBG: 50 luxury tents (sleeps ~120), west coast location 25min north of Canggu.

**Actual wishlist pricing (AUD sell, 2-night, 100 guests):**

| Component | AUD | IDR (×10,600) |
|-----------|-----|---------------|
| Resort buyout (50 tents, 2 nights) | $20,003 | 212,032,000 |
| Event fee | $2,543 | 26,956,000 |
| Banjar fee | $424 | 4,494,000 |
| F&B food (International Buffet, 100 pax) | $8,052 | 85,351,000 |
| F&B drinks (Beer & Cocktails, 100 pax) | $7,204 | 76,362,000 |
| **Venue + F&B total** | **$38,226** | **405,195,000** |
| Guest tent payback (49 tents × 2 nights) | −$18,606 | −197,224,000 |
| **Couple venue OOP** | **$19,620** | **207,971,000** |

- Per tent/night: A$200 buyout (~2.1M IDR)
- Guest payback: A$190/tent/night (~2.0M IDR) — nearly offsets full buyout
- F&B per head: A$153 (food $81 + drinks $72) = ~1,617,000 IDR
- Min F&B spend: 60-90M IDR (weekday/weekend)
- External caterer (Ravela Catering)
- All ~100 guests can stay on-site

---

## 5. The Core Tension

We need a price that satisfies three parties:

| Party | What They Want |
|-------|---------------|
| **Daisy (Oshom)** | At or above her 2026 budget ADR of 4,028,398 /room/night |
| **Selina (Sales)** | Guest room rate at or below 3,500,000 /room/night sell |
| **Couple** | Total out-of-pocket comparable to or better than Vedas (~217M - 265M) |

**Solution: Decouple the buyout rate from the guest room rate.**

- Buyout rate = what Oshom receives per room (based on ADR) → **satisfies Daisy**
- Guest room rate = what guests pay per room (based on market) → **satisfies Selina**
- The gap between the two = the couple's "exclusivity premium" → **keeps couple competitive with Vedas**

---

## 6. Recommended Pricing

### 6a. Accommodation Buyout (Full Hotel — 18 Rooms)

Priced at Oshom's 2026 budget ADR of ~4,000,000 net/room/night (rounded for clean numbers):

| | Per Room/Night | 2-Night Total (18 rooms) | 3-Night Total (18 rooms) |
|--|---------------|------------------------|------------------------|
| **Oshom net** | 4,000,000 | 144,000,000 | 216,000,000 |
| **Buy (+ 21% tax)** | 4,840,000 | **174,240,000** | **261,360,000** |
| **Sell (+ 15% BL)** | 5,694,118 | **204,988,235** | **307,482,353** |

**Check — Oshom revenue per night (buy):**
- 2-night: 174,240,000 / 2 = **87,120,000 per night**
- 3-night: 261,360,000 / 3 = **87,120,000 per night**
- vs 2026 budget of 44,108,879 → **1.97× their normal night** ✓
- vs 2025 actual of 25,935,019 → **3.36× their normal night** ✓

### 6b. Venue / Event Fee

Same pricing chain as everything else — Oshom net + 21% tax + 15% BL.

| | Amount |
|--|--------|
| **Oshom net** | 33,057,851 |
| **Buy (+ 21% tax)** | **40,000,000** |
| **Sell (+ 15% BL)** | **47,058,824** |

Starting point is 40M buy (incl tax). This is a **potential discount lever** — can be reduced to close a sale (e.g. drop to 30M or waive entirely for off-peak dates). Oshom's published venue buyout is 100,000,000 /day, so 40M for a multi-day wedding is very reasonable.

### 6c. Food & Beverage (Per Head, 80 Guests)

Using Oshom's actual published menu prices as the net base:

#### Base Tier — Food Only + Cash Bar

Couple pays for food. Guests buy their own drinks at the bar.

| Item | Oshom Net | Buy (incl tax) | Sell | × 80 Guests (Buy) | × 80 Guests (Sell) |
|------|-----------|---------------|------|-------------------|-------------------|
| Food (Family Style 3-course) | 600,000 | 726,000 | 854,118 | 58,080,000 | 68,329,412 |
| Drinks | Cash bar — guests pay Oshom directly | | | — | — |
| **Total per head** | **600,000** | **726,000** | **854,118** | **58,080,000** | **68,329,412** |

#### Standard Tier — Food + Silver Drinks (Spirits)

| Item | Oshom Net | Buy (incl tax) | Sell | × 80 (Buy) | × 80 (Sell) |
|------|-----------|---------------|------|-----------|------------|
| Food (Family Style) | 600,000 | 726,000 | 854,118 | 58,080,000 | 68,329,412 |
| Drinks (Silver) | 550,000 | 665,500 | 782,941 | 53,240,000 | 62,635,294 |
| **Total per head** | **1,150,000** | **1,391,500** | **1,637,059** | **111,320,000** | **130,964,706** |

#### Premium Tier — BBQ/Western Food + Gold Drinks (Cocktails)

| Item | Oshom Net | Buy (incl tax) | Sell | × 80 (Buy) | × 80 (Sell) |
|------|-----------|---------------|------|-----------|------------|
| Food (BBQ Family Style) | 850,000 | 1,028,500 | 1,210,000 | 82,280,000 | 96,800,000 |
| Drinks (Gold) | 650,000 | 786,500 | 925,294 | 62,920,000 | 74,023,529 |
| **Total per head** | **1,500,000** | **1,815,000** | **2,135,294** | **145,200,000** | **170,823,529** |

### 6d. Guest Room Rate (What Guests Pay)

Set independently from the buyout. Based on Selina's market feedback:

| | Amount |
|--|--------|
| **Guest room rate (sell)** | **3,500,000 /room/night** |
| Guest offset (17 rooms × 2 nights) | 119,000,000 |
| Guest offset (17 rooms × 3 nights) | 178,500,000 |

Guest room payments go directly to reduce the couple's buyout obligation. BaliLove's commission is already built into the buyout sell price — no double-dipping on guest rooms.

---

## 7. Per-Wedding Totals

### 7a. 2-Night Wedding, 80 Guests

| Component | Oshom Net | Buy (incl tax) | Sell (couple pays) | BL Commission |
|-----------|-----------|---------------|-------------------|---------------|
| Accommodation buyout | 144,000,000 | 174,240,000 | 204,988,235 | 30,748,235 |
| Venue/event fee | 33,057,851 | 40,000,000 | 47,058,824 | 7,058,824 |
| F&B Base (food + cash bar) | 48,000,000 | 58,080,000 | 68,329,412 | 10,249,412 |
| **TOTAL — BASE** | **225,057,851** | **272,320,000** | **320,376,471** | **48,056,471** |
| | | | | |
| F&B Standard (food + Silver) | 92,000,000 | 111,320,000 | 130,964,706 | 19,644,706 |
| **TOTAL — STANDARD** | **269,057,851** | **325,560,000** | **383,011,765** | **57,451,765** |
| | | | | |
| F&B Premium (BBQ + Gold) | 120,000,000 | 145,200,000 | 170,823,529 | 25,623,529 |
| **TOTAL — PREMIUM** | **297,057,851** | **359,440,000** | **422,870,588** | **63,430,588** |

### 7b. 3-Night Wedding, 80 Guests

| Component | Oshom Net | Buy (incl tax) | Sell (couple pays) | BL Commission |
|-----------|-----------|---------------|-------------------|---------------|
| Accommodation buyout | 216,000,000 | 261,360,000 | 307,482,353 | 46,122,353 |
| Venue/event fee | 33,057,851 | 40,000,000 | 47,058,824 | 7,058,824 |
| F&B Base (food + cash bar) | 48,000,000 | 58,080,000 | 68,329,412 | 10,249,412 |
| **TOTAL — BASE** | **297,057,851** | **359,440,000** | **422,870,588** | **63,430,588** |
| | | | | |
| F&B Standard | 92,000,000 | 111,320,000 | 130,964,706 | 19,644,706 |
| **TOTAL — STANDARD** | **341,057,851** | **412,680,000** | **485,505,882** | **72,825,882** |
| | | | | |
| F&B Premium | 120,000,000 | 145,200,000 | 170,823,529 | 25,623,529 |
| **TOTAL — PREMIUM** | **369,057,851** | **446,560,000** | **525,364,706** | **78,804,706** |

---

## 8. Couple's Out-of-Pocket (After Guest Room Offsets)

Guest room payments reduce the couple's buyout obligation.

### 8a. 2-Night Weddings

| F&B Tier | Total Sell | Guest Offset (17 rooms) | Couple Out-of-Pocket |
|----------|-----------|------------------------|---------------------|
| Base (cash bar) | 320,376,471 | -119,000,000 | **201,376,471** |
| Standard | 383,011,765 | -119,000,000 | **264,011,765** |
| Premium | 422,870,588 | -119,000,000 | **303,870,588** |

### 8b. 3-Night Weddings

| F&B Tier | Total Sell | Guest Offset (17 rooms) | Couple Out-of-Pocket |
|----------|-----------|------------------------|---------------------|
| Base (cash bar) | 422,870,588 | -178,500,000 | **244,370,588** |
| Standard | 485,505,882 | -178,500,000 | **307,005,882** |
| Premium | 525,364,706 | -178,500,000 | **346,864,706** |

---

## 9. Oshom vs Vedas vs BBG — Competitor Head-to-Head

> **Updated March 5:** Vedas guest accommodation corrected (guests stay free). BBG added from actual wishlists. Oshom shown at both current and revised $35K pricing.

### Venue Profiles

| | Oshom | Villa Vedas | Bali Beach Glamping |
|--|-------|-------------|---------------------|
| Type | Boutique hotel | Luxury beachfront villa | Glamping resort |
| Accommodation | 18 rooms | 7 bedrooms (sleeps 16) | 50 tents (sleeps ~120) |
| All guests on-site | **Yes** (~36 pax) | No — max 16 on-site | **Yes** (up to 120) |
| Guests pay for accom. | **Yes** (3.5M/night) | **No** — included in buyout | **Yes** (~2M/tent/night) |
| In-house kitchen | **Yes** | No (Saffron Catering) | No (Ravela Catering) |
| Min F&B spend | **None** | 40M IDR net | 60–90M IDR |
| BYO alcohol | Yes | Yes (villa) | TBC |

### Total Couple Cost (2-Night Wedding, AUD)

*From actual BaliLove wishlists. BL vendor services (planner, MUA, photo, DJ, celebrant, styling, sound, cake, transport) are ~A$16,173 across all venues.*

| Component | Oshom (Current §6-14) | Oshom ($35K §16) | Villa Vedas | BBG |
|-----------|----------------------|------------------|-------------|-----|
| Guests | 80 | 80 | 80 | 100 |
| Buyout (2 nights) | $19,338 | $14,768 | $6,272 | $20,003 |
| Event + banjar fee | $4,439 | $0 (rolled in) | $4,153 | $2,967 |
| F&B | $12,355 (Standard) | $14,479 (Premium) | $12,206 | $15,256 |
| **Venue + F&B** | **$36,132** | **$29,247** | **$22,631** | **$38,226** |
| Guest accom. offset | −$11,226 | −$10,086 | $0 (free) | −$18,606 |
| **Venue cost to couple** | **$24,906** | **$19,161** | **$22,631** | **$19,620** |
| BL vendor services | ~$16,173 | ~$16,154 | ~$16,173 | ~$16,173 |
| **Total couple cost** | **~$41,080** | **$35,315** | **~$38,804** | **~$35,793** |

### Key Takeaways

**At current pricing (~$41K),** Oshom is the most expensive option — $2.3K above Vedas and $5.3K above BBG. This is why the current pricing doesn't work.

**At $35K revised pricing ($35,315 actual wishlist),** Oshom is the cheapest option:
- **$3,500 cheaper than Vedas** ($35.3K vs $38.8K)
- **$500 cheaper than BBG** ($35.3K vs $35.8K)
- With **Premium F&B** (BBQ + Gold cocktails) — Standard would be even cheaper
- 18 private hotel rooms vs 7 villa bedrooms (Vedas) or 50 shared tents (BBG)

> **Confirmed from actual BaliLove wishlist:** Oshom at A$35,315 includes Premium F&B (80 pax), full hotel buyout (2 nights), event fee rolled in, 17-room guest payback at 3.5M/night. This is not a model — it's a real wishlist price.

**Why Oshom wins at $35K:**
- Only venue where the couple gets a full private hotel at the lowest total cost
- Guests pay a reasonable room rate (3.5M/night) which offsets the buyout
- No minimum F&B spend removes friction — Vedas requires 40M, BBG requires 60–90M
- In-house kitchen = better quality control and flexibility vs external caterers

---

## 10. Revenue Comparison for Daisy (Per Night)

| Scenario | Oshom Revenue/Night (Buy, incl tax) | vs 2026 Budget |
|----------|-------------------------------------|---------------|
| Normal night (2026 budget) | 44,108,879 | 1.0× |
| Wedding — Base (cash bar) | 136,160,000 | **3.09×** |
| Wedding — Standard | 162,780,000 | **3.69×** |
| Wedding — Premium | 179,720,000 | **4.07×** |

Even the cheapest wedding option delivers **~3× their normal nightly revenue.**

### Plus: Guest In-Resort Spend (Non-Wedding Days)

Guests staying on-site (~35 pax across 18 rooms) spend an estimated **500,000 /person/day** on food, drinks, and incidentals on non-wedding days. This goes directly to Oshom — not through BaliLove.

| | 2-Night (1 extra day) | 3-Night (2 extra days) |
|--|----------------------|----------------------|
| On-site guests | ~35 | ~35 |
| Daily spend per guest | 500,000 | 500,000 |
| **Extra Oshom revenue** | **17,500,000** | **35,000,000** |

This is **on top of** the wedding revenue above and **on top of** cash bar revenue on the wedding day itself.

**On weak months** (Nov at 29.8% occ, ~13.5M/night actual):

| Scenario | Revenue/Night | vs Weak Month Actual |
|----------|--------------|---------------------|
| Wedding — Base | 136,160,000 | **10.1×** |

---

## 11. Annual Projection (50 Weddings)

Mix: 33 × 2-night + 17 × 3-night
F&B mix assumption: 50% base, 35% standard, 15% premium

### Revenue to Oshom (Buy, incl tax)

| Component | Annual |
|-----------|--------|
| Accommodation buyout (33 × 174.2M + 17 × 261.4M) | 10,293,720,000 |
| Venue fees (50 × 40M) | 2,000,000,000 |
| F&B (weighted avg ~93M buy/wedding × 50) | 4,650,000,000 |
| **Total to Oshom (through BL)** | **16,943,720,000** |
| | |
| Guest in-resort spend (non-wedding days) | ~1,172,500,000 |
| Cash bar on wedding days (est. 35 guests × 300K) | ~525,000,000 |
| **Total incl. direct Oshom revenue** | **~18,641,220,000** |

### Displacement Cost

50 weddings = 117 wedding days (33×2 + 17×3). These days would otherwise earn normal hotel revenue.

| Basis | Displaced Revenue |
|-------|------------------|
| 2026 budget (117 days × 44,108,879) | 5,160,739,000 |
| 2025 actual (117 days × 25,935,019) | 3,034,397,000 |

### Net Incremental

| | vs 2026 Budget | vs 2025 Actual |
|--|---------------|----------------|
| Wedding revenue | 16,943,720,000 | 16,943,720,000 |
| Displaced | -5,160,739,000 | -3,034,397,000 |
| **Net incremental** | **11,782,981,000** | **13,909,323,000** |
| **% increase on room revenue** | **+73%** | **+147%** |

**Note:** The net incremental above is based only on revenue through BaliLove. Including guest in-resort spend (~1.17B) and wedding day cash bar (~525M), total incremental is closer to **+84% vs 2026 budget**.

### BaliLove Annual Commission

| | Annual |
|--|--------|
| Total sell (50 weddings) | ~19,933,788,000 |
| Total buy | ~16,943,720,000 |
| **BL commission** | **~2,990,068,000** |

---

## 12. Calendar Impact

| Metric | Value |
|--------|-------|
| Wedding days per year | 117 (32% of calendar) |
| Normal hotel days | 248 (68% of calendar) |
| Wedding day occupancy | 100% (guaranteed) |
| Normal day occupancy (2026 budget) | 61% |

Weddings target weak months (Nov-Apr) where occupancy is 30-50%. Displacement cost is lowest when weddings are needed most.

---

## 13. What Each Party Gets

### Daisy (Oshom)

- Per-room rate at her 2026 budget ADR: **4,000,000 net/room/night** ✓
- 100% occupancy on wedding days vs 61% average ✓
- Revenue per wedding night: **3-4× normal** ✓
- +73% annual revenue increase (net of displacement) ✓
- Guest in-resort spend (~1.17B/yr) + cash bar (~525M/yr) on top ✓
- Only uses 32% of calendar — 68% remains normal hotel ✓

### Couple

- Full private hotel (18 rooms, all guests on-site) ✓
- Out-of-pocket comparable to Vedas (264M standard) or cheaper (201M base) ✓
- Cash bar option removes drink cost risk ✓
- BYO alcohol available ✓
- No minimum spend — just pick a food tier ✓
- Guest room rate: 3,500,000/night — reasonable ✓

### Selina (Sales)

- Guest room rate at 3,500,000 (at ceiling, not above) ✓
- Cash bar option = easiest selling point ✓
- No minimum spend = removes friction ✓
- Oshom Base package undercuts Vedas on OOP ✓
- Full hotel proposition vs villa = clear differentiator ✓

### BaliLove

- ~2,990,000,000 annual commission from Oshom alone ✓
- ~48-79M commission per wedding ✓
- Exclusive venue partnership ✓
- Portfolio diversification beyond Vedas ✓

---

## 14. Summary Table — What to Present

### Per-Wedding Price Card (IDR)

| | 2-Night Base | 2-Night Std | 2-Night Prem | 3-Night Base | 3-Night Std | 3-Night Prem |
|--|-------------|-------------|-------------|-------------|-------------|-------------|
| Buyout (sell) | 204,988,235 | 204,988,235 | 204,988,235 | 307,482,353 | 307,482,353 | 307,482,353 |
| Venue fee (sell) | 47,058,824 | 47,058,824 | 47,058,824 | 47,058,824 | 47,058,824 | 47,058,824 |
| F&B (sell, 80 pax) | 68,329,412 | 130,964,706 | 170,823,529 | 68,329,412 | 130,964,706 | 170,823,529 |
| **Total sell** | **320,376,471** | **383,011,765** | **422,870,588** | **422,870,588** | **485,505,882** | **525,364,706** |
| Guest offset (17 rooms) | -119,000,000 | -119,000,000 | -119,000,000 | -178,500,000 | -178,500,000 | -178,500,000 |
| **Couple OOP** | **201,376,471** | **264,011,765** | **303,870,588** | **244,370,588** | **307,005,882** | **346,864,706** |

### What Oshom Receives Per Wedding (Buy, incl tax)

| | 2-Night Base | 2-Night Std | 2-Night Prem | 3-Night Base | 3-Night Std | 3-Night Prem |
|--|-------------|-------------|-------------|-------------|-------------|-------------|
| **Total buy** | **272,320,000** | **325,560,000** | **359,440,000** | **359,440,000** | **412,680,000** | **446,560,000** |
| **Per night** | **136,160,000** | **162,780,000** | **179,720,000** | **119,813,333** | **137,560,000** | **148,853,333** |

---

## 15. Resolved & Remaining Questions

### Resolved

1. **Venue fee** — 40M buy (incl tax). Goes to Oshom via the standard chain (net + 21% tax + 15% BL). This is a **discount lever** — can be reduced or waived to close a sale on off-peak dates.

2. **BYO alcohol / Corkage** — No corkage. Too small an amount to fight over. Guests can bring their own and we'll serve it, but with a reasonable limit. The cash bar is the primary model — guests buy drinks at Oshom's bar prices.

3. **Non-wedding day F&B** — Cash bar for guests by default. Aim to upsell functions (welcome dinner, recovery brunch) but no mandatory F&B on non-event days. Cash bar revenue goes to Oshom directly.

4. **Guest room rate discounting** — We'd suggest to Daisy that off-peak dates should have discounted guest room rates. Look at which months are struggling (Nov, Apr, Mar) and price guest rooms lower to drive bookings. This makes weddings more attractive in weak months and helps fill the calendar.

### Still Open

1. **Guest room rate of 3,500,000 sell** — is this the standard rate to present, with discounting available for off-peak? Or should we set a range (e.g. 2,500,000 - 3,500,000 depending on season)?

2. **F&B for non-wedding days in a 3-night package** — do we price welcome dinner / recovery brunch separately, or include a simple option in the package? Currently only the main wedding day F&B is costed.

3. **BYO limit** — what's reasonable? E.g. "up to 5 bottles of spirits" or "enough for the bridal party"? Need to agree something that doesn't undercut Oshom's bar revenue significantly.

4. **Minimum F&B spend** — we've said "no minimum" as a selling point, but internally the target is ~40M for wedding day, ~20M for other days. Do we communicate this to Daisy as a guideline, or keep it truly open?

---

## 16. Revised Scenarios — $35K and $40K Targets

> **Date:** March 5, 2026
> **Context:** Selina call confirmed current pricing (~$43K AUD total) is above market. Vedas ~$38K, BBG ~$36K. 70% of leads budget $30–35K AUD. Reworked at two price targets below.

### Key Changes vs Sections 6–14

1. **Event fee rolled into buyout** — no separate venue charge (Selina's ask)
2. **Price set by working backwards** from couple's total wedding cost in AUD
3. **Buyout rate is the variable** — F&B stays at Oshom's published menu prices

### Method

```
Couple total (AUD) × 10,600              = Total IDR
Total IDR − BL vendor services (189M)    = Couple venue OOP
Couple venue OOP + guest offset (119M)   = Venue sell (buyout + F&B)
Venue sell − F&B sell                     = Buyout sell (event fee rolled in)
Buyout sell × 0.85                        = Buyout buy (incl 21% tax)
Buyout buy ÷ 1.21                         = Oshom net (buyout)
Oshom net ÷ 18 rooms ÷ 2 nights          = Net per room per night
```

**Fixed inputs:**

| Input | Value |
|-------|-------|
| AUD → IDR | 10,600 |
| BL vendor services (sell, per wedding) | 189,000,000 |
| Guest offset (17 rooms × 3.5M × 2 nights) | 119,000,000 |
| F&B Standard sell (Family Style + Silver, 80 pax) | 130,965,000 |
| F&B Premium sell (BBQ + Gold, 80 pax) | 170,824,000 |
| F&B Standard buy | 111,320,000 |
| F&B Premium buy | 145,200,000 |

---

### 16a. Scenario A: $35,000 AUD (371,000,000 IDR)

#### Reverse Calculation

| Step | Standard F&B | Premium F&B |
|------|-------------|-------------|
| Couple total | 371,000,000 | 371,000,000 |
| − BL vendors (189M) | −189,000,000 | −189,000,000 |
| = Couple venue OOP | 182,000,000 | 182,000,000 |
| + Guest offset (119M) | +119,000,000 | +119,000,000 |
| = Venue sell | 301,000,000 | 301,000,000 |
| − F&B sell | −130,965,000 | −170,824,000 |
| = **Buyout sell** | **170,035,000** | **130,176,000** |
| Buyout buy (×0.85) | 144,530,000 | 110,650,000 |
| Oshom net (÷1.21) | 119,446,000 | 91,447,000 |
| **Net per room/night** | **3,318,000** | **2,540,000** |

**Standard (3.3M net/room/night):** Matches Oshom's 2025 actual ADR (3.31M). Pitch: "Guaranteed at your proven rate, 100% occupancy."

**Premium (2.5M net/room/night):** Below 2025 ADR — not recommended as standalone. Only viable if the room rate is set at the Standard level and Premium F&B is an upsell that increases the couple's total above $35K.

#### Per-Wedding Revenue — $35K Standard (2-Night, 80 Guests)

| Component | Oshom Net | Buy (incl tax) | Sell | BL Commission |
|-----------|-----------|---------------|------|---------------|
| Buyout (event fee rolled in) | 119,446,000 | 144,530,000 | 170,035,000 | 25,505,000 |
| F&B Standard (80 pax) | 92,000,000 | 111,320,000 | 130,965,000 | 19,645,000 |
| **Total** | **211,446,000** | **255,850,000** | **301,000,000** | **45,150,000** |

| Metric | Value |
|--------|-------|
| Oshom revenue per night (buy) | **127,925,000** |
| vs 2026 budget night (44,109,000) | **2.90×** |
| vs 2025 actual night (25,935,000) | **4.93×** |
| Couple venue OOP (after guest offset) | **182,000,000** |
| Couple total incl vendors | **371,000,000 ($35,000 AUD)** |

---

### 16b. Scenario B: $40,000 AUD (424,000,000 IDR)

#### Reverse Calculation

| Step | Standard F&B | Premium F&B |
|------|-------------|-------------|
| Couple total | 424,000,000 | 424,000,000 |
| − BL vendors (189M) | −189,000,000 | −189,000,000 |
| = Couple venue OOP | 235,000,000 | 235,000,000 |
| + Guest offset (119M) | +119,000,000 | +119,000,000 |
| = Venue sell | 354,000,000 | 354,000,000 |
| − F&B sell | −130,965,000 | −170,824,000 |
| = **Buyout sell** | **223,035,000** | **183,176,000** |
| Buyout buy (×0.85) | 189,580,000 | 155,700,000 |
| Oshom net (÷1.21) | 156,677,000 | 128,677,000 |
| **Net per room/night** | **4,352,000** | **3,574,000** |

**Standard (4.4M net/room/night):** 8% above 2026 target ADR (4.03M). Exceeds Daisy's budget goal — strong position.

**Premium (3.6M net/room/night):** Close to 2026 target ADR. Defensible as the entry rate when Premium is framed as the standard offering.

#### Per-Wedding Revenue — $40K Standard (2-Night, 80 Guests)

| Component | Oshom Net | Buy (incl tax) | Sell | BL Commission |
|-----------|-----------|---------------|------|---------------|
| Buyout (event fee rolled in) | 156,677,000 | 189,580,000 | 223,035,000 | 33,455,000 |
| F&B Standard (80 pax) | 92,000,000 | 111,320,000 | 130,965,000 | 19,645,000 |
| **Total** | **248,677,000** | **300,900,000** | **354,000,000** | **53,100,000** |

| Metric | Value |
|--------|-------|
| Oshom revenue per night (buy) | **150,450,000** |
| vs 2026 budget night (44,109,000) | **3.41×** |
| vs 2025 actual night (25,935,000) | **5.80×** |
| Couple venue OOP (after guest offset) | **235,000,000** |
| Couple total incl vendors | **424,000,000 ($40,000 AUD)** |

---

### 16c. Guest In-Resort Spend (Non-Wedding Days)

Unchanged from Section 10 — goes directly to Oshom, not through BaliLove.

| | 2-Night (1 extra day) | 3-Night (2 extra days) |
|--|----------------------|----------------------|
| On-site guests | ~35 | ~35 |
| Daily spend per guest | 500,000 | 500,000 |
| **Extra Oshom revenue** | **17,500,000** | **35,000,000** |

Annual (50 weddings): ~1,172,500,000

---

### 16d. Annual Projections — 50 Weddings

Mix: 33 × 2-night + 17 × 3-night (117 wedding days)
F&B mix: 70% standard, 30% premium (no cash bar — avg ~121M buy/wedding)
Room rate derived from Standard F&B calculation, applied across all tiers.

**3-night buyout (same room rate as 2-night):**

| | $35K Rate | $40K Rate |
|--|----------|----------|
| Net/room/night | 3,318,000 | 4,352,000 |
| 3-night buyout net (×18×3) | 179,172,000 | 235,008,000 |
| 3-night buyout buy (×1.21) | 216,798,000 | 284,360,000 |

#### Revenue to Oshom (Buy, Incl Tax)

| Component | $35K Scenario | $40K Scenario | Current Doc |
|-----------|--------------|--------------|-------------|
| Buyout — 33 × 2-night | 4,769,490,000 | 6,256,140,000 | — |
| Buyout — 17 × 3-night | 3,685,566,000 | 4,834,120,000 | — |
| **Buyout total** | **8,455,000,000** | **11,090,000,000** | **12,294,000,000**¹ |
| F&B (50 × ~121M avg) | 6,074,000,000 | 6,074,000,000 | 6,074,000,000 |
| **Total to Oshom (through BL)** | **14,529,000,000** | **17,164,000,000** | **18,368,000,000** |
| | | | |
| Guest in-resort spend | ~1,173,000,000 | ~1,173,000,000 | ~1,173,000,000 |
| **Total incl. direct revenue** | **~15,702,000,000** | **~18,337,000,000** | **~19,541,000,000** |

¹ Current doc buyout total includes venue fees (2,000,000,000)

#### Displacement & Net Incremental

| | $35K | $40K | Current Doc |
|--|------|------|-------------|
| Oshom revenue (through BL) | 14,529,000,000 | 17,164,000,000 | 18,368,000,000 |
| Displaced revenue (117 × 44.1M) | −5,161,000,000 | −5,161,000,000 | −5,161,000,000 |
| **Net incremental** | **9,368,000,000** | **12,003,000,000** | **13,207,000,000** |
| **% increase on room revenue** | **+58%** | **+75%** | **+82%** |

#### BaliLove Annual Commission

| | $35K | $40K | Current Doc |
|--|------|------|-------------|
| Total sell (50 weddings) | ~17,093,000,000 | ~20,193,000,000 | ~21,609,000,000 |
| Total buy | ~14,529,000,000 | ~17,164,000,000 | ~18,368,000,000 |
| **BL commission** | **~2,564,000,000** | **~3,029,000,000** | **~3,241,000,000** |

---

### 16e. Comparison Table — All Price Points (2-Night Standard)

**Oshom revenue metrics:**

| Metric | $35K Oshom | $40K Oshom | Current Doc |
|--------|-----------|-----------|-------------|
| Oshom net/room/night | 3,318,000 | 4,352,000 | 4,918,000¹ |
| vs 2025 ADR (3.31M) | Match | +32% | +49% |
| vs 2026 target (4.03M) | −18% | +8% | +22% |
| Revenue/night (buy, all items) | 127,925,000 | 150,450,000 | 162,780,000 |
| vs normal night (2026 budget) | 2.90× | 3.41× | 3.69× |
| BL commission/wedding | 45,150,000 | 53,100,000 | 57,452,000 |
| Annual Oshom rev (50 wed) | 14,529M | 17,164M | 18,368M |
| Annual net incremental | +9,368M | +12,003M | +13,207M |
| Annual BL commission | 2,564M | 3,029M | 3,241M |

¹ Current doc net/room/night includes venue fee equivalent (4.0M buyout + 0.9M venue fee per room/night)

**Competitive position (from actual BaliLove wishlists):**

| Metric | Oshom @ $35K | Oshom @ $40K | Oshom Current | Villa Vedas | BBG |
|--------|-------------|-------------|---------------|-------------|-----|
| Couple total (AUD) | **$35,315** | **~$40,000** | ~$41,100 | ~$38,800 | ~$35,800 |
| Couple venue OOP (AUD) | $19,161 | ~$24,000 | $24,906 | $22,631 | $19,620 |
| Rooms/units on-site | **18** | **18** | **18** | 7 | 50 |
| All guests on-site | **Yes** | **Yes** | **Yes** | No | Yes |
| Guests pay for accom. | Yes | Yes | Yes | No (free) | Yes |
| In-house kitchen | **Yes** | **Yes** | **Yes** | No | No |
| Min F&B spend | **None** | **None** | **None** | 40M IDR | 60–90M IDR |
| **Cheapest option?** | **Yes** | No (≈ Vedas) | No (most expensive) | — | — |

---

### 16f. The Volume Argument — Why Start at $35K

The $35K price point isn't about leaving money on the table. It's about speed to volume. **Give us control of pricing and the sales process, and we'll have 20 weddings booked by end of May.**

#### The pitch to Daisy:

> "How would you feel if you had 20 weddings booked by the end of May? Give us full control of the pricing and how we sell it. Our team has 300+ weddings of data — we know exactly who's buying what, how to overcome every objection, and we will work up the total revenue and profit to your business over time."

#### The condition: BL controls pricing and sales

This only works if BaliLove has **full autonomy** over:
- **Pricing** — we set the couple's total, the F&B tier options, and the seasonal adjustments
- **Sales process** — we handle leads end-to-end (enquiry → site visit → close → coordination)
- **Upselling** — we decide when and how to push Premium F&B, extra nights, vendor upgrades

Oshom's role: deliver the product (rooms, food, service). BL's role: fill the calendar and maximise revenue per wedding.

#### Why 20 weddings at $35K beats 10 weddings at $43K:

Mix scaled proportionally: 20 wed = 13×2-night + 7×3-night; 10 wed = 7×2-night + 3×3-night.

| Metric | 20 weddings @ $35K | 10 weddings @ $43K |
|--------|-------------------|-------------------|
| Wedding days used | 47 (13% of calendar) | 23 (6% of calendar) |
| Oshom revenue (through BL) | ~5,826,000,000 | ~3,619,000,000 |
| Displaced revenue | −2,073,000,000 | −1,015,000,000 |
| Net incremental | **+3,753,000,000** | **+2,604,000,000** |
| BL commission | ~1,028,000,000 | ~639,000,000 |
| Guest in-resort spend | ~473,000,000 | ~228,000,000 |

**$35K at 20 weddings delivers 44% more net revenue than $43K at 10 weddings** — using only 13% of the calendar vs 6%.

#### Why lower price = more weddings:

1. **70% of leads budget $30–35K** — at $35K we capture the mass market, not just the top 30%
2. **Beats every named competitor** — cheaper than Vedas ($38K), comparable to BBG ($36K), with 18 rooms vs 8
3. **BL sales team knows the objections** — 300+ weddings of experience closing deals. Lower price = fewer objections = faster close rate
4. **Conversion rate scales non-linearly** — dropping from $43K to $35K (−19%) could double or triple conversion from the same lead pool
5. **Speed matters** — 20 bookings by end of May means Oshom sees revenue impact in 2026, not 2027

#### The growth path:

| Phase | Timeline | Target | Price Strategy | Revenue Lever |
|-------|----------|--------|----------------|---------------|
| Launch | Now → May 2026 | 20 bookings | $35K base | Prove the model, fill calendar fast |
| Scale | Jun → Dec 2026 | 35–40 total | $35K off-peak, $40K peak | Season-based pricing, F&B upsells |
| Optimise | 2027+ | 50+ per year | Dynamic pricing | Full revenue optimization, marketplace |

**Phase 1 is about proof of concept and speed.** Get 20 weddings booked fast, deliver exceptional service, build a portfolio of real Oshom weddings. The revenue optimisation comes in Phase 2 once we have:
- Actual booking data (which months convert, which F&B tiers sell, which vendors add value)
- Testimonials and photos from real Oshom weddings
- Social proof to justify premium pricing on peak dates
- Data-backed confidence to push the average wedding value up from $35K toward $40K+

---

### 16g. Pitch Talking Points

#### $35,000 — The Volume Play

- "We beat every competitor on price. $35K total vs Vedas at $38K, BBG at $36K."
- Oshom room rate matches their 2025 actual ADR — no discounting, just guaranteed occupancy
- Even at the lowest price: **2.9× normal nightly revenue** per wedding night
- Target 50+ weddings at maturity — fill the weak months (Nov–Apr) where occupancy drops to 30%
- Annual net incremental to Oshom: **+9.4B IDR (+58% on room revenue)** at 50 weddings
- Plus ~1.2B in direct guest in-resort spend that bypasses BaliLove
- **Risk:** Room rate sits at 2025 levels — Daisy may push back on not hitting 2026 targets. Counter: "100% occupancy on wedding days vs 47% average in 2025. You're making more money, just through volume."

#### $40,000 — The Premium Play

- "Still $2K cheaper than Vedas. Premium positioning with a full private hotel."
- Room rate of 4.4M net **exceeds** Daisy's 2026 target ADR by 8% — easy sell to GM
- **3.4× normal nightly revenue** per wedding night
- Annual net incremental to Oshom: **+12.0B IDR (+75% on room revenue)** at 50 weddings
- Plus ~1.2B in direct guest in-resort spend
- **Risk:** Fewer leads convert at $40K — 70% of market budgets $30–35K. Counter: target the 30% premium segment, quality over quantity

#### Recommended: Lead with $35K, Season-Based Uplift

**Off-peak (Nov–Apr):** $35K — "Beat everyone on price, fill empty rooms"
**Peak (May–Oct):** $40K — "Premium positioning, higher margin, still under Vedas"

This gives Selina two price cards and a simple story: the couple gets a better deal in the quiet season, and Oshom fills their weakest months first.
